...
class Reward:
    def __init__(self, single_rate: int = 4, quinella_rate: int = 12, trifecta_rate: int = 28) -> None:
        """得点計算
    ...
    def reward_point(self, df: pd.DataFrame, result: list[int], game_id: int | None = None) -> int:
        """keimaの獲得pointを計算する
        ...
        columnは、"game_id", "ticket_type", "one", "two", "three" を想定。
        ...
        for _, ticket in single_tickets.iterrows():
            if ticket['one'] == result[0]:
                point += self.single_rate
        
        for _, ticket in quinella_tickets.iterrows():
            if ticket['one'] == result[0] and ticket['two'] == result[1]:
                point += self.quinella_rate
        
        for _, ticket in trifecta_tickets.iterrows():
            if ticket['one'] == result[0] and ticket['two'] == result[1] and ticket['three'] == result[2]:
                point += self.trifecta_rate
